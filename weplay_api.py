...
default_avatar = (
    "https://static-prod.weplay.tv/2019-09-13/1e4b1366e9876217059a373ed871f504.png"
)
...
async def get_ids(session: ClientSession, lobby_id: str, side: str):
    response = await request(
        session,
        f"{base_url}/csgo-match-processor/v1/matches/{lobby_id}",
        "get",
        verify_ssl=False,
    )
...
async def get_nicknames_and_avatars(session: ClientSession, ids: dict):
    ...
    response = await request(
        session,
        f"{base_url}/user-management-service/v3/users?filter__user_id__in={ids_str}",
        "get",
        verify_ssl=False,
    )
    users = response["data"]
    ...
    for steam_id, user in zip(ids["steam_ids"], users):
        if "avatar_path" in user.keys():
            ...
            data[steam_id] = {"nickname": user["nickname"], "avatar": default_avatar}
    return data
